...
npm install
mbt build
cf deploy ./mta_archives/btp-resource-consumption_2.0.0.mtar -e mtaext_notifications.mtaext
```
...
npm install
mbt build
cf deploy ./mta_archives/btp-resource-consumption_2.0.0.mtar
```
...
- **SAC Insufficient Privileges**: In case the SAC story can not load because of insufficient privileges, use the `HANA Cloud Cockpit` to assign the missing privileges to the user used to connect to the HDI. A known suspect here is the `_SYS_BI::BIMC_PROPERTIES` table which needs `SELECT` privileges.
